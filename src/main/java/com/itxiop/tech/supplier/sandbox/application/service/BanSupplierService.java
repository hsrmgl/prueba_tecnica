...
public class BanSupplierService implements BanSupplierUseCase {
    ...
    public void ban(int duns) {
        Supplier supplier = supplierRepo.findByDuns(duns)
            .orElseThrow(() -> new SupplierNotFoundException("Supplier not found: " + duns));
        if (!supplier.isOnProbation()) {
            throw new SupplierCannotBeBannedException("Supplier cannot be banned: not on probation");
        }
        supplierRepo.save(supplier.ban());
    }
}
